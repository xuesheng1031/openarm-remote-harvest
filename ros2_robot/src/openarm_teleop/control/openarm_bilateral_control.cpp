...
#include <ros/teleop_ros_publisher.hpp>
#include <thread>
...
std::string teleop_config_path(const std::string &name) {
    try {
        return ament_index_cpp::get_package_share_directory("openarm_teleop") + "/config/" + name;
...
int main(int argc, char **argv) {
    try {
        ...
        YamlLoader leader_loader(teleop_config_path("leader.yaml"));
        YamlLoader follower_loader(teleop_config_path("follower.yaml"));
        ...
        AdminThread admin_thread(leader_state, follower_state, control_leader, control_follower,
                                 FREQUENCY);
        TeleopRosPublisher ros_pub(leader_state, follower_state, arm_side, 100.0);

        leader_thread.start_thread();
        follower_thread.start_thread();
        admin_thread.start_thread();
        ros_pub.start_thread();
