...
int main(int argc, char** argv) {
    try {
        ...
        while (keep_running) {
            ...
            // Scale down to reduce upward drift (model mass may be slightly high)
            const double grav_scale = 0.95;
            for (auto& t : grav_torques) t *= grav_scale;


            // Print joint 5 and joint 7 state and gravity torque at about 10 Hz.
            if (frame_count % 100 == 0 && grav_torques.size() > 6) {
                std::cout << "[Joint 5] q[4] = " << arm_joint_positions[4]
                          << " rad, dq[4] = " << arm_joint_velocities[4]
                          << " rad/s, grav_torques[4] = " << grav_torques[4] << " Nm | "
                          << "[Joint 7] q[6] = " << arm_joint_positions[6]
                          << " rad, dq[6] = " << arm_joint_velocities[6]
                          << " rad/s, grav_torques[6] = " << grav_torques[6] << " Nm"
                          << std::endl;
            }
