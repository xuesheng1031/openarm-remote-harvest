...
- A random 64-bit `session_id` changes on each process start. `sequence` increases
  within one session. Receivers reject duplicate and out-of-order sequence numbers.
...
`obs_timestamp_ns`, `action_timestamp_ns`, `applied_action_sequence`, control state,
and fault bits. `action_timestamp_ns == 0` and `applied_action_sequence == 0` mean
that no remote action has entered a control cycle.
